...
export async function generateLogo(category, name, accentHex) {
  ...
  const prompt = `A modern professional horizontal logo for a ${category} business. The business name "${name}" must be written clearly and correctly in elegant, clean, highly legible typography as the centerpiece. Beside or above the name, include a small tasteful icon: ${icon}. Color scheme: ${accentHex || '#2563eb'} with dark charcoal text. Premium brand wordmark, balanced horizontal layout, fully transparent background, crisp readable lettering, spelled exactly as "${name}".`;
